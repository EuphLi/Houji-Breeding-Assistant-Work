...
本仓库的 Issue 和 PRD 以本地 Markdown 文件形式存放在 `.scratch/` 目录下。详见 `docs/agents/issue-tracker.md`。

### Triage labels

使用默认的标准标签词汇（`needs-triage`、`needs-info`、`ready-for-agent`、`ready-for-human`、`wontfix`）。详见 `docs/agents/triage-labels.md`。

### Domain docs

单一上下文仓库：根目录下放置 `CONTEXT.md`，架构决策存放在 `docs/adr/` 中。详见 `docs/agents/domain.md`。
