...
def _is_direct_breeding_workbench_tool_run(meta: dict) -> bool:
    return (
        str((meta or {}).get("source") or "").strip() == BREEDING_WORKBENCH_SOURCE
        and str((meta or {}).get("preferred_tool") or "").strip()
        == BREEDING_WORKBENCH_DIRECT_TOOL
    )
...
async def process_agent_run(ctx, run_id: str):
    ...
    logger.info(
        f"Worker picked up run: run_id={run_id} thread_id={run.thread_id} "
        f"agent_id={run.agent_id} status={run.status}"
    )
    ...
    # }
    # 新逻辑：先恢复前端传入的育种工作台结构化上下文，再叠加 worker 运行时可信字段。
    # 这样可以向“受控育种工作台执行链路”推进，同时避免前端伪造 user_id/thread_id/request_id。
    ...
    logger.info(
        f"Run meta context restored: run_id={run_id} thread_id={run.thread_id} "
        f"source={meta.get('source')} trait={meta.get('trait')} "
        f"preferred_tool={meta.get('preferred_tool')} "
        f"allowed_tools_count={len(meta.get('allowed_tools') or []) if isinstance(meta.get('allowed_tools'), list) else 0} "
        f"has_breeding_context={bool(meta.get('breeding_context'))}"
    )
    if _is_direct_breeding_workbench_tool_run(meta):
        logger.info(
            f"Breeding workbench direct tool route enabled: run_id={run_id} "
            f"preferred_tool={meta.get('preferred_tool')}"
        )
