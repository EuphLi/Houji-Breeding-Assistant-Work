...
# """
KG_QUERY_DESCRIPTION = """
This tool queries triples from a knowledge graph.

MUST FOLLOW:
- The knowledge graph entity names are in English.
- The `query` argument MUST be English.
- If the user asks in Chinese, first extract the key entity or concept, then translate it into English before calling this tool.
- Never pass the user's original Chinese question as `query`.
- Use concise English keywords, entity names, or aliases, not a full sentence.
- Prefer canonical English names, e.g. "United States" instead of "美国", "Apple Inc." instead of "苹果公司".

Good examples:
User: "马斯克创办了哪些公司？"
Call: query_knowledge_graph("Elon Musk companies")

User: "法国的首都是哪里？"
Call: query_knowledge_graph("France capital")

Bad examples:
query_knowledge_graph("马斯克创办了哪些公司？")
query_knowledge_graph("法国的首都是哪里？")
"""
...
def query_knowledge_graph(
    query: Annotated[
        str,
        "English keyword/entity to query the knowledge graph. Must be English. "
        "Translate Chinese user questions into concise English entity names or keywords before calling. "
        "Do not pass the user's raw input."
    ]
) -> Any:
    """查询知识图谱中的三元组信息。query 必须是英文关键词或英文实体名，不要传入中文原问题。"""
    try:
        from yuxi import graph_base
