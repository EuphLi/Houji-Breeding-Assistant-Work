...
"""
RuntimeConfigMiddleware 是当前工具注入核心
它同时处理：模型覆盖、工具覆盖、系统提示词覆盖、MCP 工具加载
...
↓
根据 context.tools 选择工具     即 context.tools → 选择工具 → 覆盖 request.tools
"""
...
class RuntimeConfigMiddleware(AgentMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        self.tools: list[Any] = []
        # 预加载工具列表（仅当启用工具覆盖时） RuntimeConfigMiddleware 初始化时，会把 YuXi 本地注册的所有工具加载进来。
        # 注意：知识库工具已移至独立的 KnowledgeBaseMiddleware
        if self.enable_tools_override:
            # breeding/tools.py 里的工具如果通过 @tool 注册成功，
            # 最终会被 get_all_tool_instances() 收集，
            # 再进入 RuntimeConfigMiddleware 的 self.tools。
            # 普通 Python 工具函数
            # ↓
            # @tool 注册
            # ↓
            # get_all_tool_instances()
            # ↓
            # RuntimeConfigMiddleware.self.tools
            # 这就是工具进入 Agent 的前半段。
            self.base_tools = get_all_tool_instances()  # Tool Registry 的入口
    ...
    async def awrap_model_call(
        ...
        """
        每次模型调用前，RuntimeConfigMiddleware 会检查当前 context 里启用了哪些 tools。
        然后从 request.tools 里保留：
        1. 已经启用的工具；2. 不归本 middleware 管理的工具。
        最后用 request.override(tools=merged_tools) 改写模型可见工具列表。
        所以，工具是否能被模型看到，不是由 allowed_tools 决定，而是由 context.tools 决定
        context.tools
        ↓
    ...
    async def get_tools_from_context(self, context) -> list:
        ...
        # 如果 context.tools 中写了某个工具名，并且这个工具名存在于 RuntimeConfigMiddleware.self.tools，那么这个工具就会被选中。
        # FIXME:YuXi 现有工具启用机制是通过 context.tools 控制的；allowed_tools 如果想变成硬约束，后续要么把它写入 context.tools，要么在 RuntimeConfigMiddleware 中额外读取 allowed_tools 并过滤工具
