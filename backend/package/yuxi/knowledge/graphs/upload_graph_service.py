...
class UploadGraphService:
    ...
    def _infer_source_file_name(self, file_path, object_name=None):
        if object_name:
            return os.path.basename(object_name)
        parsed = urlparse(file_path)
        candidate = os.path.basename(parsed.path or "")
        return candidate or "upload.jsonl"

    def _resolve_graph_metadata(self, file_path, graph_id=None, graph_name=None, source_file_name=None, object_name=None):
        resolved_source_file_name = source_file_name or self._infer_source_file_name(file_path, object_name)
        resolved_graph_id = graph_id or str(uuid4())
        resolved_graph_name = graph_name or Path(resolved_source_file_name).stem or resolved_graph_id
        return resolved_graph_id, resolved_graph_name, resolved_source_file_name

    def _process_record_props(self, record):
    ...
    async def jsonl_file_add_entity(
        ...
        try:
            if parsed_url.scheme in ("http", "https"):  # 如果是 URL
                ...
                try:
                    resolved_graph_id, resolved_graph_name, resolved_source_file_name = self._resolve_graph_metadata(
                        file_path,
                        graph_id=graph_id,
                    ...
                    if skip_embedding:
                        ...
                        warnings_list = await self.txt_add_vector_entity(
                            triples,
                            kgdb_name=kgdb_name,
    ...
    def import_triples(self, triples, kgdb_name="neo4j", graph_id=None, graph_name=None, source_file_name=None):
        """只导入三元组结构，不触发 embedding 流程"""
        assert self.driver is not None, "Database is not connected"
        self.use_database(kgdb_name)
        resolved_graph_id = graph_id or str(uuid4())
        resolved_graph_name = graph_name or (Path(source_file_name or "").stem or resolved_graph_id)
        resolved_source_file_name = source_file_name or f"{resolved_graph_name}.jsonl"
        ...
        def _create_graph(tx, data):
            ...
            for entry in data:
                ...
                h_uid = f"{resolved_graph_id}::{h_name}"
                t_uid = f"{resolved_graph_id}::{t_name}"
                r_uid = f"{resolved_graph_id}::{h_name}::{r_type}::{t_name}"
                tx.run(
                    """
                MERGE (h:Entity:Upload {uid: $h_uid})
                SET h.name = $h_name,
                    h.uid = $h_uid,
                    h.graph_id = $graph_id,
                    h.graph_name = $graph_name,
                    h.source_file_name = $source_file_name,
                    h += $h_props
                MERGE (t:Entity:Upload {uid: $t_uid})
                SET t.name = $t_name,
                    t.uid = $t_uid,
                    t.graph_id = $graph_id,
                    t.graph_name = $graph_name,
                    t.source_file_name = $source_file_name,
                    t += $t_props
                MERGE (h)-[r:RELATION {uid: $r_uid}]->(t)
                SET r.type = $r_type,
                    r.uid = $r_uid,
                    r.graph_id = $graph_id,
        ...
        with self.driver.session() as session:
            logger.info(f"Importing graph structure to {kgdb_name}, graph_id={resolved_graph_id}")
    ...
    async def txt_add_vector_entity(
        ...
        def _get_nodes_without_embedding(tx, entity_uids):
            """获取没有embedding的节点列表"""
            params = {f"param{i}": uid for i, uid in enumerate(entity_uids)}
            ...
            result = tx.run(
                f"""
            MATCH (n:Entity)
            WHERE n.uid IN [{param_placeholders}] AND n.embedding IS NULL
            RETURN n.uid AS uid, n.name AS name
            """,
                params,
            )

            return [{"uid": record["uid"], "name": record["name"]} for record in result]

        def _batch_set_embeddings(tx, entity_embedding_pairs):
            """批量设置实体的嵌入向量"""
            for entity_uid, embedding in entity_embedding_pairs:
                tx.run(
                    """
        ...
        with self.driver.session() as session:
            ...
            for entry in triples:
                ...
                if h_name:
                    all_entities[f"{resolved_graph_id}::{h_name}"] = h_name
                if t_name:
                    all_entities[f"{resolved_graph_id}::{t_name}"] = t_name

            all_entity_uids = list(all_entities)

            nodes_without_embedding = session.execute_read(_get_nodes_without_embedding, all_entity_uids)
            if not nodes_without_embedding:
                logger.info("所有实体已有embedding，无需重新计算")
                return warnings_list

            logger.info(f"需要为{len(nodes_without_embedding)}/{len(all_entity_uids)}个实体计算embedding")
            ...
            for i in range(0, total_entities, max_batch_size):
                batch_nodes = nodes_without_embedding[i : i + max_batch_size]
                logger.debug(
                    f"Processing entities batch {i // max_batch_size + 1}/"
                    f"{(total_entities - 1) // max_batch_size + 1} ({len(batch_nodes)} entities)"
                )

                try:
                    batch_names = [item["name"] for item in batch_nodes]
                    batch_embeddings = await self.aget_embedding(batch_names, batch_size=batch_size)
                except Exception as e:
                    warning_message = f"Embedding 计算失败，图谱结构已导入：{e}"
                    logger.warning(f"{warning_message}\n{traceback.format_exc()}")
                    warnings_list.append(warning_message)
                    self.save_graph_info()
                    return warnings_list

                entity_embedding_pairs = [(item["uid"], embedding) for item, embedding in zip(batch_nodes, batch_embeddings)]
    ...
    def query_sample_nodes(self, kgdb_name="neo4j", graph_id=None, num=100):
        """采样查询 upload 图谱，按 graph_id 过滤；graph_id=None 时兼容历史默认图谱"""
        assert self.driver is not None, "Database is not connected"
        assert self.is_running(), "图数据库未启动"
        self.use_database(kgdb_name)

        def query(tx, graph_id, num):
            query_str = """
            MATCH (n:Upload)
            WHERE ($graph_id IS NULL AND n.graph_id IS NULL) OR n.graph_id = $graph_id
            WITH n LIMIT $num
            OPTIONAL MATCH (n)-[r:RELATION]-(m:Upload)
            WHERE m IS NULL
               OR (($graph_id IS NULL AND m.graph_id IS NULL AND r.graph_id IS NULL)
                   OR (m.graph_id = $graph_id AND r.graph_id = $graph_id))
            RETURN
                ...
                ELSE null END AS t
            """
            results = tx.run(query_str, graph_id=graph_id, num=int(num))
            ...
            for item in results:
                ...
                if item["r"] is not None and item["t"] is not None:
                    ...
                    if r_edge and r_edge["id"] not in edge_ids:
                        formatted_results["edges"].append(r_edge)
                        edge_ids.add(r_edge["id"])

            return formatted_results

        with self.driver.session() as session:
            return session.execute_read(query, graph_id, num)
    ...
    def query_node(
        ...
        **kwargs,
        ...
        entity_to_score = {}
        for token in tokens:
            results_sim = self._query_with_vector_sim(token, kgdb_name, threshold, graph_id=graph_id)
            ...
            results_fuzzy = self._query_with_fuzzy_match(token, kgdb_name, graph_id=graph_id)
    ...
    def _query_with_fuzzy_match(self, keyword, kgdb_name="neo4j", graph_id=None):
        """模糊查询"""
        assert self.driver is not None, "Database is not connected"
        self.use_database(kgdb_name)

        def query_fuzzy_match(tx, keyword, graph_id):
            result = tx.run(
                """
            MATCH (n:Upload)
            WHERE toLower(n.name) CONTAINS toLower($keyword)
              AND (($graph_id IS NULL AND n.graph_id IS NULL) OR n.graph_id = $graph_id)
        ...
        with self.driver.session() as session:
            return session.execute_read(query_fuzzy_match, keyword, graph_id)

    def _query_with_vector_sim(self, keyword, kgdb_name="neo4j", threshold=0.9, graph_id=None):
        """向量查询"""
        assert self.driver is not None, "Database is not connected"
        self.use_database(kgdb_name)
        ...
        def query_by_vector(tx, text, threshold, graph_id):
            # 首先检查索引是否存在
            ...
            WHERE 'Upload' IN labels(similarEntity)
              AND (($graph_id IS NULL AND similarEntity.graph_id IS NULL) OR similarEntity.graph_id = $graph_id)
        ...
        with self.driver.session() as session:
            results = session.execute_read(query_by_vector, keyword, threshold=threshold, graph_id=graph_id)
            return results

    def _query_specific_entity(self, entity_name, kgdb_name="neo4j", hops=2, limit=100, graph_id=None):
        """查询指定实体三元组信息（无向关系）"""
        assert self.driver is not None, "Database is not connected"
        if not entity_name:
        ...
        def query(tx, entity_name, hops, limit, graph_id):
            try:
                query_str = """
                MATCH (n:Upload {name: $entity_name})
                WHERE (($graph_id IS NULL AND n.graph_id IS NULL) OR n.graph_id = $graph_id)
                MATCH path = (n)-[:RELATION*1..2]-(m:Upload)
                WHERE all(node IN nodes(path) WHERE (($graph_id IS NULL AND node.graph_id IS NULL) OR node.graph_id = $graph_id))
                UNWIND relationships(path) AS rel
                WITH startNode(rel) AS h, rel, endNode(rel) AS t
                RETURN DISTINCT
                    {id: elementId(h), name: h.name, properties: properties(h)} AS h,
                    {
                        id: elementId(rel),
                        type: rel.type,
                    ...
                    {id: elementId(t), name: t.name, properties: properties(t)} AS t
                LIMIT $limit
                """
                results = tx.run(query_str, entity_name=entity_name, graph_id=graph_id, limit=limit)
                ...
                for item in results:
                    h = self._process_record_props(item["h"])
                    r = self._process_record_props(item["r"])
                    t = self._process_record_props(item["t"])
        ...
        try:
            with self.driver.session() as session:
                return session.execute_read(query, entity_name, hops, limit, graph_id)
