...
async def test_txt_add_vector_entity_parsing():
    ...
    with patch("yuxi.models.select_embedding_model") as mock_select_model:
        ...
        with (
            patch("yuxi.knowledge.graphs.upload_graph_service.config", mock_config),
            patch(
                "yuxi.knowledge.graphs.upload_graph_service.get_embedding_model_info_by_id",
                return_value={"dimension": 1024},
            ),
        ):
