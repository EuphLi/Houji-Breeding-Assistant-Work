    ...
    return SimpleNamespace(
        ...
        input_payload={
            ...
            "meta": {
                ...
                "trait": "smoke_flavonoid",
                "preferred_tool": "smoke_flavonoid_breeding_advice",
                "allowed_tools": ["smoke_flavonoid_breeding_advice"],
                "breeding_context": {"goal": "筛选候选材料"},
            },
            "agent_id": "ChatbotAgent",
            "image_content": None,
            "user_id": "1",
...
@pytest.mark.asyncio
async def test_process_agent_run_passes_frontend_meta_and_runtime_fields(monkeypatch: pytest.MonkeyPatch):
    run_obj = _build_run()
    run_obj.input_payload["image_content"] = "base64-image"
    run_obj.input_payload["meta"]["user_id"] = "forged-user"
    run_obj.input_payload["meta"]["thread_id"] = "forged-thread"
    run_obj.input_payload["meta"]["request_id"] = "forged-request"
    _patch_common(monkeypatch, run_obj)
    ...
    meta = stream_kwargs["meta"]
    assert meta["source"] == "breeding-workbench"
    assert meta["trait"] == "smoke_flavonoid"
    assert meta["breeding_context"] == {"goal": "筛选候选材料"}
    assert meta["allowed_tools"] == ["smoke_flavonoid_breeding_advice"]
    assert meta["preferred_tool"] == "smoke_flavonoid_breeding_advice"
    assert meta["request_id"] == "req-1"
    assert meta["query"] == "hello"
    assert meta["agent_id"] == "ChatbotAgent"
