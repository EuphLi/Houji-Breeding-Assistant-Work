...
@pytest.mark.asyncio
async def test_create_agent_run_persists_breeding_workbench_meta(monkeypatch: pytest.MonkeyPatch):
    class FakeDB:
        async def commit(self):
            return None
    ...
    meta = {
        ...
        "allowed_tools": ["smoke_flavonoid_breeding_advice", "query_literature"],
        "breeding_context": {"material": "F2", "goal": "筛选高黄酮低烟味材料"},
    }
    ...
    assert payload["meta"]["source"] == "breeding-workbench"
    assert payload["meta"]["preferred_tool"] == "smoke_flavonoid_breeding_advice"
    assert "smoke_flavonoid_breeding_advice" in payload["meta"]["allowed_tools"]
    assert payload["meta"]["breeding_context"] == {"material": "F2", "goal": "筛选高黄酮低烟味材料"}
