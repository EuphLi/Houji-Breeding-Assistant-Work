...
from yuxi.services.sms_service import (
...
@auth.post("/sms/send", response_model=SmsSendResponse)
async def sms_send_code(request: Request, body: SmsSendRequest):
    """发送手机验证码"""
    from server.utils.user_utils import is_valid_phone_number

    if not is_valid_phone_number(body.phone_number):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="手机号格式不正确")

    forwarded = request.headers.get("x-forwarded-for")
    client_ip = forwarded.split(",")[0].strip() if forwarded else (request.client.host if request.client else "unknown")
    try:
        await send_verification_code(body.phone_number, client_ip)
...
@auth.post("/sms/login", response_model=Token)
async def sms_login(body: SmsLoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not body.code or not (4 <= len(body.code) <= 8) or not body.code.isdigit():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="验证码格式不正确")

    if not await verify_code(body.phone_number, body.code):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="验证码错误或已过期",
        )

    user_info = await login_or_register_by_phone(body.phone_number, db)

    access_token = AuthUtils.create_access_token({"sub": str(user_info["user_id"])})
