...
@graph.get("/graphs")
async def get_upload_graphs(current_user: User = Depends(get_required_user)):
    """列出 upload 类型图谱（每个上传文件一个图谱 + 兼容旧默认图谱）"""
    try:
        return {"success": True, "data": graph_base.list_upload_graphs()}
    except Exception as e:
        logger.error(f"Failed to list upload graphs: {e}")
        logger.error(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Failed to list upload graphs: {str(e)}")
...
async def get_subgraph(
    ...
    max_nodes: int | None = Query(100, description="最大节点数", ge=1),
    graph_id: str | None = Query(None, description="upload 图谱ID"),
    all: bool = Query(False, description="是否加载当前图谱全部节点"),
    ...
    """
    try:
        logger.info(f"Querying subgraph - db_id: {db_id}, label: {node_label}, graph_id: {graph_id}, all: {all}")

        graph_type = await GraphAdapterFactory.detect_graph_type(db_id, knowledge_base)
        adapter = await _get_graph_adapter(db_id)
        resolved_max_nodes = None if all and graph_type == "upload" else max_nodes
...
async def get_graph_stats(
    ...
    try:
        ...
        if db_id.startswith("kb_") or knowledge_base.is_lightrag_database(db_id):
            ...
            # Neo4j stats (直接管理的图谱)
            if graph_id:
                upload_graph = graph_base.get_upload_graph(graph_id=graph_id, kgdb_name=db_id)
                return {
                    "success": True,
                    "data": {
                        "total_nodes": upload_graph.get("node_count", 0),
                        "total_edges": upload_graph.get("edge_count", 0),
                        "entity_types": [],
                    },
                }
...
async def get_neo4j_nodes(
    ...
    num: int | None = Query(100, description="节点数量", ge=1),
    graph_id: str | None = Query(None, description="upload 图谱ID"),
    all: bool = Query(False, description="是否加载当前图谱全部节点"),
    current_user: User = Depends(get_required_user),
):
...
async def get_neo4j_node(
    ...
    response = await get_subgraph(db_id="neo4j", node_label=entity_name, graph_id=graph_id, current_user=current_user)
    return {"success": True, "result": response["data"], "message": "success"}
...
async def index_neo4j_entities(data: dict = Body(default={}), current_user: User = Depends(get_required_user)):
    """为Neo4j图谱节点添加嵌入向量索引"""
    try:
        if not graph_base.is_running():
...
async def add_neo4j_entities(
    ...
    graph_id: str | None = Body(None),
    graph_name: str | None = Body(None),
    source_file_name: str | None = Body(None),
    ...
    try:
        ...
        graph_record = await graph_base.jsonl_file_add_entity(
            file_path,
            kgdb_name,
            embed_model_name,
            batch_size,
