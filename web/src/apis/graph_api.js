...
export const unifiedApi = {
  ...
  getUploadGraphs: async () => {
    return await apiGet('/api/graph/graphs', {}, true)
  },
  ...
  getSubgraph: async (params) => {
    const { db_id, node_label = '*', max_depth = 2, max_nodes = 100, graph_id = null, all = false } = params
    ...
    queryParams.set('db_id', db_id)
    queryParams.set('node_label', node_label)
    queryParams.set('max_depth', max_depth.toString())
    if (graph_id) {
      queryParams.set('graph_id', graph_id)
    }
    if (all) {
      queryParams.set('all', 'true')
    } else if (max_nodes != null) {
      queryParams.set('max_nodes', max_nodes.toString())
    }
   ...
   */
  getStats: async (db_id, graph_id = null) => {
    if (!db_id) {
      throw new Error('db_id is required')
    }
...
export const neo4jApi = {
   ...
   */
  getSampleNodes: async (kgdb_name = 'neo4j', num = 100, graph_id = null, all = false) => {
    const queryParams = new URLSearchParams()
    queryParams.set('kgdb_name', kgdb_name)
    if (graph_id) {
      queryParams.set('graph_id', graph_id)
    }
    if (all) {
      ...
      queryParams.set('num', num.toString())
    }
   ...
   */
  queryNode: async (entity_name, graph_id = null) => {
    if (!entity_name) {
      throw new Error('entity_name is required')
    }

    const queryParams = new URLSearchParams()
    queryParams.set('entity_name', entity_name)
    if (graph_id) {
      queryParams.set('graph_id', graph_id)
    }
