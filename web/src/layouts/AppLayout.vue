...
const isEffectivelyCollapsed = computed(() => sidebarCollapsed.value || isMobile.value)
...
const mainList = computed(() => {
  ...
  // })
    if (!isLiteMode) {
      items.push({
        name: '育种工作台',
        path: '/breeding-workbench',
        activePaths: ['/breeding-workbench'],
        icon: Sprout,
        activeIcon: Sprout
      })
...
<template>
  <div
    class="app-layout"
    :class="{ 'use-top-bar': layoutSettings.useTopBar, 'sidebar-collapsed': isEffectivelyCollapsed }"
  >
    <div class="header" :class="{ 'top-bar': layoutSettings.useTopBar }">
      <div class="sidebar-brand" @click.stop>
        <router-link v-if="!isEffectivelyCollapsed" to="/agent" class="brand-link">
      ...
      <div class="nav">
        ...
        <RouterLink
          ...
          @click.stop
        >
          <a-tooltip placement="right" :open="isEffectivelyCollapsed ? undefined : false">
      ...
      <div class="foo">
        ...
        <div class="nav-item user-info" @click.stop>
          <UserInfoComponent :show-role="!isEffectivelyCollapsed">
    ...
    <!-- 移动端侧边栏 Drawer -->
    <MobileSidebarDrawer v-model:open="isMobileDrawerOpen" />
  </div>
</template>
...
.header {
  ...
  height: 100%;
  width: @sidebar-width;
  border-right: 1px solid color-mix(in srgb, var(--main-color) 7%, var(--gray-100));
  ...
  .sidebar-toggle {
    ...
    border-radius: 8px;
    background: color-mix(in srgb, var(--gray-0) 86%, var(--main-color) 4%);
    color: var(--gray-600);
    cursor: pointer;
    transition:
  ...
  .nav-item {
    ...
    &.active {
      ...
      font-weight: 600;
      color: var(--main-bright);
      box-shadow: 0 4px 12px color-mix(in srgb, var(--main-color) 6%, transparent);
    }
    ...
    &.primary-action {
      ...
      border: none;
      color: #fff;
      box-shadow: 0 6px 16px color-mix(in srgb, var(--main-color) 18%, transparent);
      transition:
        transform 0.18s ease,
        opacity 0.2s ease,
        box-shadow 0.18s ease;
    ...
    &:hover {
      ...
      background-color: var(--main-20);
      color: var(--main-bright);
      box-shadow: 0 4px 12px color-mix(in srgb, var(--main-color) 5%, transparent);
    }
