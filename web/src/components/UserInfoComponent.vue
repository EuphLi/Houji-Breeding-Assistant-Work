    ...
    <a-dropdown :trigger="['hover']" v-if="userStore.isLoggedIn">
      ...
      <template #overlay>
        <a-menu>
          ...
          <a-menu-divider />
          <a-menu-item key="docs" @click="openDocs" :icon="BookOpenIcon"  v-if="userStore.isSuperAdmin">
            <span class="menu-text">文档中心</span>
          </a-menu-item>
          <!-- 深色模式切换（已隐藏，后续计划移除深色主题功能）
          <a-menu-item
            key="theme"
            @click="toggleTheme"
          ...
          -->
          <a-sub-menu key="accent">
            <template #title>
              <Palette :size="16" />
              <span>主题色</span>
            </template>
            <a-menu-item key="accent-purple" @click="setAccentAndNotify('purple')">
              <span class="color-dot" style="background: #6b21a8"></span>
              <span class="menu-text">紫色&nbsp;</span>
              <Check v-if="themeStore.accent === 'purple'" :size="14" class="accent-check" />
            </a-menu-item>

            <a-menu-item key="accent-green" @click="setAccentAndNotify('green')">
              <span class="color-dot" style="background: #008f51"></span>
              <span class="menu-text">绿色&nbsp;</span>
              <Check v-if="themeStore.accent === 'green'" :size="14" class="accent-check" />
            </a-menu-item>

            <a-menu-item key="accent-gold" @click="setAccentAndNotify('gold')">
              <span class="color-dot" style="background: #7a4a0f"></span>
              <span class="menu-text">金色&nbsp;</span>
              <Check v-if="themeStore.accent === 'gold'" :size="14" class="accent-check" />
            </a-menu-item>
          </a-sub-menu>
...
const PaletteIcon = h(Palette, { size: '16' })
...
const setAccentAndNotify = (accent) => {
  themeStore.setAccent(accent)

  // 如果当前页面运行在 iframe 里，通知外层首页同步主题色
  if (window.parent && window.parent !== window) {
    window.parent.postMessage(
      {
        type: 'ACCENT_CHANGED_FROM_AGENT',
        accent
      },
