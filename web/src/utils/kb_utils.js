...
export function normalizeEmbeddingModelSpec(value) {
  if (!value) return ''
  ...
  const normalized = raw
    .replace(/^siliconflow-cn:/, 'siliconflow/')
    .replace(/^siliconflow:/, 'siliconflow/')
    .replace(/^siliconflow-cn\//, 'siliconflow/')
    .replace(/^Pro\/BAAI\/bge-m3$/, 'siliconflow/Pro/BAAI/bge-m3')
    .replace(/^BAAI\/bge-m3$/, 'siliconflow/BAAI/bge-m3')

  return normalized.toLowerCase()
}

export function isSameEmbeddingModelSpec(a, b) {
  return normalizeEmbeddingModelSpec(a) === normalizeEmbeddingModelSpec(b)
}
